...
private displayError(field: string, message: string): void {
  const errorElementId = this.errorIds[field as keyof ErrorElementIds]
    ...
    if (errorElementId) {
      const errorElement = document.getElementById(errorElementId) as HTMLInputElement;

      if(errorElementId){const errorElement = document.getElementById(errorElementId) as HTMLElement
     if(errorElement) {
